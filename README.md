...
Every client library request authenticates with the server using a `storeId` parameter and header field `X-API-Key`.

Example:
````java
KodyPayTerminalService api = new KodyPayTerminalService(baseAddress, apiKey);
TerminalsResponse response = api.terminals(payStoreId.toString());
````

Replace the `payStoreId`, `apiKey` and `baseAddress` with the details provided by the Kody team.
Note: it is recommended that you store the `apiKey` in a secured storage, and insert it into your code via an environment variable.
